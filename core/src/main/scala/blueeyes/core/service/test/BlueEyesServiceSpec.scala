...
class BlueEyesServiceSpec extends WordSpec with MustMatchers with BeforeAndAfterAll with blueeyes.concurrent.test.AkkaFutures with HttpReflectiveServiceList[ByteChunk] with AkkaDefaults { self =>
  ...
  protected def beforeSpec() {}
  protected def afterSpec() {}
