...
class RollSpec extends WordSpec with MustMatchers with Roll{
  "Roll: calculates nextRollTime with Never policy" in {
    val next = calendar(1)
    next.add(Calendar.YEAR, 100)

    nextRollTime(Never, 1) must equal (next.getTimeInMillis)
  }
  "Roll: calculates nextRollTime with Hourly policy" in {
    val next = calendar(1)
    next.add(Calendar.HOUR_OF_DAY, 1)

    nextRollTime(Hourly, 1) must equal (next.getTimeInMillis)
  }
  "Roll: calculates nextRollTime with Daily policy" in {
    ...
    nextRollTime(Daily, 1) must equal (next.getTimeInMillis)
  }
  "Roll: calculates nextRollTime with Weekly policy" in {
    ...
    nextRollTime(Weekly(1), 1) must equal (next.getTimeInMillis)
  }
