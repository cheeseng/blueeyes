...
class DynamicHistogramSpec extends WordSpec with MustMatchers with TimedStatFixtures {
  "DynamicHistogram" should{
    "build Histogram" in{
      ...
      result.histogram must equal (Map(15 -> 2, 18 -> 1, 21 -> 3))
    }
    ...
    "fill last missing buckets" in{
      ...
      result.histogram must equal (Map(15 -> 0, 18 -> 1, 21 -> 3))
    }
    ...
    "fill first missing buckets" in{
      ...
      clock.setNow(21100)
      result.histogram must equal (Map(15 -> 2, 18 -> 1, 21 -> 0))
    }
