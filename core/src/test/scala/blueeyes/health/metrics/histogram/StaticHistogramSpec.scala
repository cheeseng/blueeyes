...
class StaticHistogramSpec extends WordSpec with MustMatchers{
  "builds Histogram" in{
    ...
    histogram must equal (Map(1 -> 4, 4 -> 1, 7 -> 0, 10-> 3))
  }
  ...
  "builds Histogram with predified bucket size" in{
    ...
    histogram must equal (Map(100 -> 2, 103 -> 5, 106 -> 0, 109 -> 0, 112-> 7))
  }
}
