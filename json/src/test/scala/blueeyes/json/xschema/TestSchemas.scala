...
package data.fringe {
  ...
  class DataProductSerializationExamples extends WordSpec with MustMatchers {
    ...
    "Serialization of ConstantSingleton has non-zero information content" in {
      Decomposers.ConstantSingletonDecomposer.decompose(ExampleProductData.ExampleConstantSingleton) must not be theSameInstanceAs (JObject(Nil))
    }
  ...
  class DataCoproductSerializationExamples extends WordSpec {
    
  }
  class DataConstantsSerializationExamples extends WordSpec with MustMatchers {
    ...
    "Serialization of constant ConstantBool has non-zero information content" in {
      Constants.ConstantBool.serialize must not equal JObject(Nil)
    }
